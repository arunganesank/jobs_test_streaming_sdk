    ...
    with open(os.path.join(args.output, case["case"] + CASE_REPORT_SUFFIX), "r") as file:
        ...
        if test_case_report["gray_artifacts_detected"] and test_case_report["test_status"] != "error":
            if args.test_group in MC_CONFIG["second_win_client"]:
                test_case_report["message"] += ["Gray artifacts could be detected on First Windows client"]
            else:
                test_case_report["message"] += ["Gray artifacts could be detected on Windows client"]
